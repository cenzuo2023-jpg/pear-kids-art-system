...
# Roster: 剩余课时
replacement_roster_hours = r'''<!-- 剩余课时 (Notion 显眼状态标签) -->
...
text = re.sub(r'<!-- 剩余课时[^\n]+\n\s*<td class="py-3 px-4 whitespace-nowrap font-mono font-bold">\s*<span :class="s\.remainHours[^>]+>\s*余 \{\{ s\.remainHours \}\} 节\s*</span>\s*</td>', replacement_roster_hours, text)
...
text = re.sub(r'<!-- 画币积分 -->\s*<td class="py-3 px-3 whitespace-nowrap">\s*<button @click\.stop="openIndividualPointModal[^>]+>\s*<span>⭐️\{\{ s\.points \|\| 0 \}\}</span>\s*<span class="text-\[9px\] opacity-60 ml-0\.5">分</span>\s*</button>\s*</td>', replacement_roster_points, text)
