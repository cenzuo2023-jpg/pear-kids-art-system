...
replacement = r'''const getClassSortValue = (name) => {
...
pattern = r'const activeClasses = computed\(\(\) => \{\s*return classes\.value\.filter\(c => c\.status !== \'archived\'\);\s*\}\);\s*const archivedClasses = computed\(\(\) => \{\s*return classes\.value\.filter\(c => c\.status === \'archived\'\);\s*\}\);'
